...
export const BottomNav = () => {
  ...
  // Hide bottom nav on auth page, messages page, video watch page, and movion pages (has own nav)
  if (
  ...
  return (
    <nav className="bottom-nav shadow-lg">
      <div className="grid grid-cols-6 h-14 sm:h-16 max-w-screen-xl mx-auto px-1">
        {navItems.map(({ icon: Icon, label, path }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <div className={`p-1 sm:p-1.5 rounded-full transition-all duration-200 ${isActive ? 'bg-primary/10' : ''}`}>
                  <Icon className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform ${isActive ? 'scale-110' : ''}`} />
                </div>
                <span className="text-[10px] sm:text-xs font-medium truncate w-full text-center px-0.5">{label}</span>
                {isActive && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 sm:w-12 h-0.5 bg-gradient-primary rounded-b-full" />
                )}
              </>
            )}
          </NavLink>
